...
if __name__ == '__main__':
    ...
    G = nx.Graph()
    pre_node_name = None
    m = m_list[1]   # 好きな評価値を選んでね！(ブラウザで選択してもらう)
    p = p_list[0]   # 好きなパラメータを選んでね！(ブラウザで選択してもらう)
    m_num = None
    p_num = None
    ...
    for keys in json_dict.keys():
        ...
        for i in hash_dict.keys():
            ...
            if m not in hash_dict[i]['metrics']: continue  # jsonのメトリクスリストにmxがなかった場合
            if p not in hash_dict[i]['parameter']: continue  # jsonのメトリクスリストにmyがなかった場合
            m_num = hash_dict[i]['metrics'].index(m)  # mxの位置
            p_num = hash_dict[i]['parameter'].index(p)  # myの位置
            print(keys + '_' + i)
            node_name = keys + '_' + i
            G.add_node(node_name)  # Add node
            G.nodes[node_name]["pos"] = (float(hash_dict[i]['parameter_change'][p_num].split()[2]),
                                         int(hash_dict[i]['metrics_value'][m_num][:-1]))  # Add node position
    ...
    fig.update_xaxes(title=p)  # X軸タイトルを指定
    fig.update_yaxes(title=m)  # Y軸タイトルを指定
