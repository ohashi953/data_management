...
if __name__ == '__main__':
    ...
    for keys in json_dict.keys():
        ...
        for i in hash_dict.keys():
            ...
            if m not in hash_dict[i]['metrics']: continue  # jsonのメトリクスリストにmがなかった場合
            m_num = hash_dict[i]['metrics'].index(m)  # mの位置
            metrics = hash_dict[i]['metrics'][m_num] + "[" + hash_dict[i]['metrics_value'][m_num][-1:] + "]" # Y軸タイトル
            node_name = keys + '_' + i
            G.add_node(node_name)  # Add node
            G.nodes[node_name]["pos"] = (hash_dict[i]['author_date'], int(hash_dict[i]['metrics_value'][m_num][:-1]))  # Add node position
            if pre_h:  # Add edge
                pre_f = hash_dict[i]['original_file'][-1]
                if pre_f:
