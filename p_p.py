...
if __name__ == '__main__':
    ...
    for keys in json_dict.keys():
        ...
        for i in hash_dict.keys():
            ...
            G.nodes[node_name]["pos"] = (float(hash_dict[i]['parameter_change'][px_num].split()[2]),
                                         float(hash_dict[i]['parameter_change'][py_num].split()[2]))  # Add node position
    ...
    fig.update_xaxes(title=px)  # X軸タイトルを指定
    fig.update_yaxes(title=py)  # Y軸タイトルを指定
